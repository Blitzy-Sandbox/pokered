...
| Block out-of-bounds LOL glitch (blockoobLG) | **DISCLOSED (R1-excluded).** The publicly documented oobLG variant that writes the opponent-species byte from an alternate source able to reach control-character indices, including Mew's `$15`; once set, `InitOpponent` reads that byte and the setup routes indices below 200 to a wild battle. Catalogued only as excluded contrast in [mf-10](methods/mf-10-blockooblg.md). | `[engine/battle/core.asm:L6647-6650]`, `[engine/battle/core.asm:L6674-6676]` |
...
| charmap (character map) | The text-encoding table. Name-enterable glyphs are HIGH bytes (space `$7f`, `A` = `$80` … `Z` = `$99`, `a` = `$a0`, `0` = `$f6`); the only sub-`$7f` byte a name carries is the `"@"` terminator `$50`, and bytes `$00`–`$17` are text-control codes. Consequently Mew's index byte `$15` is not a typeable name character — though this bounds only the typed bytes, since positions past the terminator (and, for a custom name capped at seven typed characters, every byte past the seventh) hold indeterminate residual, not typed input. | `[constants/charmap.asm:L1]`, `[constants/charmap.asm:L12]`, `[constants/charmap.asm:L63]`, `[constants/charmap.asm:L92]`, `[constants/charmap.asm:L117]`, `[constants/charmap.asm:L126]`, `[constants/charmap.asm:L187]`, `[engine/menus/naming_screen.asm:L243-250]` |
...
| Name buffer (`wPlayerName`) | The 11-byte player-name storage buffer, sized by `NAME_LENGTH` (11). A custom name types at most seven characters (`PLAYER_NAME_LENGTH - 1` = 7), so only its first seven bytes are player-chosen — the rest being the `$50` terminator and residual RAM — while each of the three build-specific preset names is fully deterministic ROM data. In the disclosed Cinnabar glitch its bytes are the data reinterpreted as a wild-encounter list; the full byte-level proof that neither typed nor preset names encode `$15` is in [mf-2](methods/mf-2-cinnabar-name-buffer.md). | `[ram/wram.asm:L1715]`, `[constants/text_constants.asm:L3]`, `[engine/menus/naming_screen.asm:L243-250]`, `[data/player/names_list.asm:L3-9]` |
...
| Out-of-bounds LOL glitch (oobLG) | **DISCLOSED (R1-excluded).** The publicly documented mass-farming technique that writes the opponent-species byte from a fixed screen tile; it can set indices below 200 but is bounded away from the control-character range that contains Mew's `$15`. Catalogued only as excluded contrast in [mf-9](methods/mf-9-out-of-bounds-lol-glitch.md). | `[engine/battle/core.asm:L6647-6650]`, `[constants/charmap.asm:L1]` |
...
| Remaining HP glitch | **DISCLOSED (R1-excluded).** The publicly documented technique that reinterprets a party Pokémon's remaining-HP byte as its species index — so it can yield any index, including Mew's `$15` — set up with a guaranteed catch-rate-255 base capture and box reordering. Catalogued only as excluded contrast in [mf-8](methods/mf-8-remaining-hp-glitch.md). | `[engine/items/item_effects.asm:L305-308]`, `[constants/pokemon_constants.asm:L30]` |
