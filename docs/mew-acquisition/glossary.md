...
| charmap (character map) | The text-encoding table. Name-enterable glyphs are HIGH bytes (space `$7f`, `A` = `$80` … `Z` = `$99`, `a` = `$a0`, `0` = `$f6`), while bytes `$00`–`$17` are text-control codes. Consequently Mew's index byte `$15` is not a typeable name character. | `[constants/charmap.asm:L1]`, `[constants/charmap.asm:L63]`, `[constants/charmap.asm:L92]`, `[constants/charmap.asm:L117]`, `[constants/charmap.asm:L126]`, `[constants/charmap.asm:L187]` |
| `DebugNewGameParty` | A `_DEBUG`-only data table that lists `db MEW, 5` / `db MEW, 20`. It is the only in-ROM grant of Mew and is dead (unreferenced) code in the retail builds, so it is not an input-reachable method. | `[engine/debug/debug_party.asm:L15-24]` |
...
| SRAM (Save RAM) | The battery-backed cartridge save RAM that persists the save file and the Pokémon storage boxes. | `[ram/sram.asm]` |
