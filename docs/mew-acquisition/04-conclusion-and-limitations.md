...
A word-boundary search for the `MEW` symbol across the source tree at this pinned commit — spanning `constants/`, `data/`, `engine/`, `home/`, `ram/`, `scripts/`, `maps/`, and `text/` — makes the gap explicit; the word "exhaustive" here is bounded to those audited repository surfaces at this checkout, not to a claim about every possible build configuration. The table below records every occurrence the audit returns and whether it constitutes an obtainable placement.
...
flowchart TD
    ...
    subgraph DEAD["Dead reader path (retail)"]
        E["DebugNewGameParty db MEW - byte in retail ROM, reader dead"]
    end
