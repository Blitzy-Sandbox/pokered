...
Requirement R4 (exhaustiveness) requires the CoolTrainer♀ / move-0x00 family (row 7) to be adjudicated, not merely listed. It is adjudicated in full in its own method chapter, [MF-7: Move-name-buffer overflow](methods/mf-7-move-name-buffer-overflow.md); the summary below records why it is EXCLUDED and links to that chapter for the complete seven-section treatment.

- Mechanism (repository-grounded): `FormatMovesString` stages each move's name into the fixed 20-byte `wNameBuffer` through `GetName` `[home/names2.asm:L86-88]` — `NAME_BUFFER_LENGTH` equals 20 `[constants/text_constants.asm:L8]` — then copies characters out of it into `wMovesString` until a `"@"`, with no length cap on that copy `[engine/battle/misc.asm:L17-24]`. When the staged name contains no `"@"`, the copy over-reads past `wNameBuffer`, which shares its WRAM region by `UNION` with in-battle move data `[ram/wram.asm:L899-906]`, and over-writes past `wMovesString` `[ram/wram.asm:L1563]` — the surface the disclosed "-" / CoolTrainer♀ corruption drives.
- Verdict: EXCLUDED under R1 because the family is in the public corpus (row 7), and only partially input-reachable, because the overflow requires a disclosed unterminated-name glitch move as a precondition rather than a clean-save controller sequence. The routines are ordinary game code `[engine/battle/misc.asm:L2]`, but that precondition is not an inputs-only path to species `$15`.
- Full adjudication — including the inputs-only verdict and the note that the overrun does not itself rewrite the enemy-species byte — is in [MF-7](methods/mf-7-move-name-buffer-overflow.md).
